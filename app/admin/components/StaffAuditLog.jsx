...
// app/admin/components/StaffAuditLog.jsx
// Shared admin audit-log view for the food and ram modules, mirroring the
// exhibition audit page but reading the shared audit_log table (rows tagged
// per module). Every post / deliver / rollback (food) or approve / cancel /
// restore / deliver (ram) event with the order's member, branch/hub, cycle
// and total, so admins can review staff actions from one place.
//
// The two modules share every column; only the endpoint, labels, action list
// and location vocabulary differ — hence one component, two thin pages.
...
const ACTION_META = {
  post: { tone: 'info', Icon: History },
  deliver: { tone: 'success', Icon: Truck },
  rollback: { tone: 'warning', Icon: RotateCcw },
  approve: { tone: 'success', Icon: CheckCircle2 },
  restore: { tone: 'warning', Icon: RotateCcw },
  cancel: { tone: 'danger', Icon: XCircle },
}
...
export default function StaffAuditLog({ module, apiPath, title = 'Audit Log', subtitle, actions = [], locationLabel = 'Branch' }) {
  ...
  const locationParam = module === 'ram' ? 'location_id' : 'branch_id'
  ...
  const load = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        const nextLocation = typeof opts.locationId === 'string' ? opts.locationId : locationId
        const qs = new URLSearchParams({
          limit: String(Math.max(1, nextPageSize)),
          offset: String(Math.max(0, (nextPage - 1) * nextPageSize)),
          ...(nextTerm ? { q: nextTerm } : {}),
          ...(nextAction ? { action: nextAction } : {}),
          ...(nextLocation ? { [locationParam]: nextLocation } : {}),
        })
        const res = await fetch(`${apiPath}?${qs}`, { cache: 'no-store', signal: ctl.signal })
        const json = await safeJson(res, apiPath)
        if (!res.ok || !json?.ok) throw new Error(json?.error || 'Failed to load audit log')
        setEvents(json.events || [])
        setTotal(Number(json.total ?? 0))
        const locs = json.locations || json.branches || []
    ...
    [page, pageSize, term, action, locationId, apiPath, locationParam, safeJson]
  )
  ...
  const runSearch = (nextTerm = term, nextAction = action, nextLocation = locationId) => {
    setPage(1)
    load({ page: 1, term: nextTerm, action: nextAction, locationId: nextLocation })
  }
  ...
  return (
    <ProtectedRoute allowedRoles={['admin']}>
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mb-6">
          <h1 className="text-h2 font-bold tracking-tight text-fg">{title}</h1>
        ...
        <Card className="mb-4 p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-end">
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    setAction(next)
                    runSearch(term, next, locationId)
                  }}
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-subtext" htmlFor="audit-location">
                  {locationLabel}
                </label>
                <Select
                  id="audit-location"
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-2 px-4 py-12 text-center">
              ...
              <p className="max-w-sm text-xs text-muted">
                {module === 'ram'
                  ? 'Approvals, cancellations, restores and deliveries on ram orders will appear here.'
                  : 'Posts, deliveries and rollbacks on food orders will appear here.'}
              </p>
            </div>
          ) : (
            <Table>
              ...
              <TBody>
                {events.map((ev) => {
                  ...
                  const o = ev.order || {}
                  const locationName = module === 'ram' ? o.hub_name : o.branch_name
                  return (
                    <TR key={ev.id}>
                      ...
                      <TD>
                        <div className="font-medium text-fg">{o.order_id || ev.order_id || '—'}</div>
                        {o.status && (
                      ...
                      <TD className="text-subtext">
                        {locationName || '—'}
                        {o.cycle_name ? <div className="text-xs text-muted">{o.cycle_name}</div> : null}
                      </TD>
                      <TD>
